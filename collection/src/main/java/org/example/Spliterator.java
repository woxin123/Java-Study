...
public interface Spliterator<T> {
    /**
     * 该方法会对每一个元素进行处理，如果没有元素处理则返回false，否则返回true
    ...
    /**
     * 将一个Spliterator。分割的Spliterator被用于每个子线程进行处理，从而达到并发的效果
    ...
    /**
     * 返回forEachRemain()遍历元素的估计值，如果这个结果是无穷大，未计数的，那么返回
     * Long.MAX_VALUE，
    ...
    /**
     * 返回此Spliterator及其元素的一组特征值，这些结果表示为（ORDERED ， DISTINCT ， SORTED ， SIZED ，
     *  NONNULL ， IMMUTABLE ， CONCURRENT ， SUBSIZED ）的或运算
    ...
    /**
     * 返回true，如果Spliterator包含所有给定的特征值。
    ...
    /**
     * 如果这个Spilterator的数据源是可排序的，如果数据源是自然排序，返回null。
     * 如果数据源不是SORTED，则抛出IllegalStateException
     ...
     * {@link java.util.Spliterator.OfInt} for {@code Integer}.
     *
     ...
     * @since 1.8
     */
    public interface OfInt extends java.util.Spliterator.OfPrimitive<Integer, IntConsumer, java.util.Spliterator.OfInt> {
     ...
     * @since 1.8
     */
    public interface OfLong extends java.util.Spliterator.OfPrimitive<Long, LongConsumer, java.util.Spliterator.OfLong> {
     ...
     * @since 1.8
     */
    public interface OfDouble extends java.util.Spliterator.OfPrimitive<Double, DoubleConsumer, java.util.Spliterator.OfDouble> {
