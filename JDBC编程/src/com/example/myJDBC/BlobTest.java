...
public class BlobTest {
    ...
    static {
        try {
            ...
            // 创建两个PreparedStatement对象，用于查询指定的图片，查询所有的图片
    ...
    public void init() throws SQLException {
        ...
        jp.add(filePath);
        jp.add(browerBn);
        browerBn.addActionListener(event->{
            // 显示文件对话框
    ...
    public void filListModel() throws SQLException {
        try (
            //执行查询
            ResultSet rs = queryAll.executeQuery()) {
            // 先清除所有的元素
            imageModel.clear();
            // 把所有查询记录添加到ListModel中
            while (rs.next()) {
                imageModel.addElement(new ImageHolder(
    ...
    class ExtensionFileFilter extends FileFilter{
        ...
        public boolean accept(File f) {
            ...
            if (f.isDirectory())
                return true;
            // 将文件名全部转为小写（全部转为小写后比较，用于忽略文件名大小写）
